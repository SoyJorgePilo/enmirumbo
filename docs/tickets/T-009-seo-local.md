...
- [ ] Existen páginas indexables por giro (`/plomeria`) y giro+colonia (`/plomeria-haciendas-de-tizayuca`) generadas desde el catálogo cerrado (PRD §8): listan negocios publicados con ese giro asignado, con título "«Giro» en Tizayuca" / "«Giro» en «Colonia», Tizayuca"; las combinaciones sin negocios publicados no se indexan ni enlazan (evitar thin content), sin romper con 404 confuso
- [ ] Las páginas de giros deportivos cubren E4-3 con el mismo mecanismo ("clases de futbol en Tizayuca" debe tener página aterrizable); si el título natural del giro deportivo pide otra fórmula, la spec la define
- [ ] Cada ficha publicada emite Schema.org LocalBusiness (JSON-LD) con lo que el PRD §8 considere honesto emitir (nombre, categoría/giro, colonia — no domicilio exacto), y las expectativas realistas del PRD v0.8 quedan citadas en la spec
- [ ] Existe `sitemap.xml` generado con home, categorías, giros/giro+colonia con contenido, y fichas publicadas; se actualiza sin intervención manual
- [ ] Existe `robots.txt` (E5-5): permite lo público, excluye `/admin`, `/buscar` y lo que la spec determine
- [ ] Metadata por página: título y descripción propios para listados de categoría, páginas de giro y fichas (hoy heredan la del sitio); Open Graph en la ficha (título, descripción, imagen — la foto del negocio si T-008 ya se mergeó, o el fallback que la spec defina)
- [ ] Los slugs de giro no chocan con rutas propias ni con slugs de categoría (hoy comparten la raíz `/[categoria]`): la spec resuelve la convivencia de rutas sin ambigüedad
- [ ] Todo Server Components, sin dependencias nuevas salvo justificación, sin degradar el presupuesto <2s en 4G
