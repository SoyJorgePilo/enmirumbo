...
- [ ] La home muestra el buscador arriba de las categorías (PRD §6.2: "buscador + categorías como botones grandes"); funciona sin JS de cliente (form GET)
- [ ] Existe la página de resultados con las mismas tarjetas del listado (reutilizadas de T-004), y estado vacío útil que ofrece las categorías como alternativa
- [ ] La búsqueda cubre nombre del negocio, palabras clave de "¿Qué ofreces?" y giros asignados; solo negocios `publicado`
- [ ] Insensible a mayúsculas y acentos, con coincidencia parcial: "plomero" encuentra al de "plomería" y al de "plomeria"; "futbol" encuentra al club aunque escriba "fútbol"
- [ ] Consulta vacía o de puro espacio no busca (regresa o muestra la home/resultados con aviso); términos hostiles (unicode raro, muy largos) se acotan sin error 500
- [ ] La página de resultados no es indexable (meta robots noindex: las URLs con query no son las páginas SEO de E5)
- [ ] Mobile-first, Server Components, sin dependencias nuevas (la normalización de acentos se resuelve con lo que ya existe — ver `src/lib/slug.ts`)
