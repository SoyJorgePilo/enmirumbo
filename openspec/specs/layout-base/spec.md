...
Ninguna superficie servida del sitio —páginas públicas, panel, metadata, vista previa al compartir, textos legales y mensajes de WhatsApp que el sitio arma— DEBE contener **la marca anterior** (el nombre con el que se lanzó el directorio, sustituido por "EnMiRumbo" el 2026-09-04 en T-019, en cualquier combinación de mayúsculas y minúsculas, suelta o pegada a un dominio) **ni la forma compuesta** que pega la localidad al wordmark como apellido ("EnMiRumbo" seguido de "Tizayuca", sin coma ni descriptor), que el fundador descartó junto con el nombre viejo. La verificación automática del proyecto DEBE incluir un guardián que revise el código de las superficies del sitio y falle nombrando el archivo cuando cualquiera de las dos reaparezca, para que un literal nuevo escrito por otro change no las resucite sin que nadie se entere. Las dos cadenas exactas que el guardián busca viven en su propia suite de pruebas y en el change archivado `renombrar-sitio-enmirumbo`; esta spec no las reproduce, porque describe lo que el sitio dice hoy.
...
- **WHEN** alguien agrega al sitio un texto, un título o un mensaje de WhatsApp con el nombre anterior del directorio
- **THEN** la verificación automática falla e indica en qué archivo y en qué línea apareció

#### Scenario: alguien vuelve a pegarle la localidad a la marca

- **WHEN** alguien escribe en el sitio un literal que pone "Tizayuca" pegado detrás del wordmark, como si fuera parte del nombre
...
- **WHEN** se corre la verificación con los devlogs, los ADR, los tickets cerrados y los changes archivados nombrando a la marca anterior
